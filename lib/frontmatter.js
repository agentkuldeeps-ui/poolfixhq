...
import { dimensionsFor, ratingFromScores } from './scoring'
...
export const SCHEMA = {
  ...
  /* --------------------------------------------------------------- extras */
  /**
   * The score breakdown. Keys must match the dimensions published for the
   * category in lib/scoring.js, and the headline `rating` on the product must
   * equal the weighted mean — both enforced below. See lib/scoring.js for why.
   */
  scores: { type: 'object', required: false, default: null },
  /** Non-obvious details a buyer would otherwise miss. Rendered as its own block. */
  buyersMiss: { type: 'string[]', required: false, default: [] },
...
export function validateFields(data, file) {
    ...
    switch (rule.type) {
      ...
      case 'object':
        if (typeof value !== 'object' || Array.isArray(value))
          fail(file, `"${key}" must be a mapping, not a list`)
        out[key] = value
        break
  ...
  /* ------------------------------------------------------- score integrity */
  if (out.scores) {
    const dims = dimensionsFor(out.category)
    const expected = dims.map((d) => d.key)
    const given = Object.keys(out.scores)

    const unknown = given.filter((k) => !expected.includes(k))
    if (unknown.length)
      fail(
        file,
        `scores contains dimension(s) not published for "${out.category}": ${unknown.join(', ')}. ` +
          `Valid dimensions are: ${expected.join(', ')} (see lib/scoring.js).`,
      )

    const missing = expected.filter((k) => out.scores[k] === undefined)
    if (missing.length)
      fail(
        file,
        `scores is missing dimension(s): ${missing.join(', ')}. Every product in a category is ` +
          'scored on every dimension, or the scores are not comparable.',
      )

    for (const [k, v] of Object.entries(out.scores)) {
      const n = Number(v)
      if (!Number.isFinite(n) || n < 0 || n > 10)
        fail(file, `scores.${k} must be a number between 0 and 10, got ${v}`)
    }

    // THE transparency guarantee: the headline rating is not an independent
    // opinion, it is the arithmetic result of the published breakdown. If they
    // disagree, one of them is being fudged. See lib/scoring.js.
    const implied = ratingFromScores(out.category, out.scores)
    const declared = out.products?.[0]?.rating
    if (declared !== undefined && declared !== null && Math.abs(declared - implied) > 0.051) {
      fail(
        file,
        `products[0].rating is ${declared} but the weighted score breakdown implies ${implied}. ` +
          'Either fix the rating or change a dimension score and defend it in the text -- ' +
          'the headline number has to be derivable from the published breakdown.',
      )
    }
  }
